    ...
    if not paused:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                cv2.putText(frame, f'{index_to_middle_length:.2f}', (int((index_x + middle_x) / 2), int((index_y + middle_y) / 2)),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                cv2.putText(frame, f'{middle_to_thumb_length:.2f}', (int((middle_x + thumb_x) / 2), int((middle_y + thumb_y) / 2)),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                cv2.putText(frame, f'{thumb_to_index_length:.2f}', (int((thumb_x + index_x) / 2), int((thumb_y + index_y) / 2)),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)

                # Write the coordinates on the screen
                cv2.putText(frame, f'Thumb: ({thumb_x}, {thumb_y})', (thumb_x, thumb_y),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(frame, f'Index: ({index_x}, {index_y})', (index_x, index_y),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(frame, f'Middle: ({middle_x}, {middle_y})', (middle_x, middle_y),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
